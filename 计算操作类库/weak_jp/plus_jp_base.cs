                ...
                else if (Base_Config_Cjba._备案数据.Contains(dt.Columns[j].ColumnName))
                {
                    ...
                    if (temp_cjba_sz != null)
                    {
                        switch (dt.Columns[j].ColumnName)
                        {
                            ...
                            case Base_Config_Cjba.上周_建面均价: { dr1[dt.Columns[j].ColumnName] = temp_cjba_sz.Sum(m => m[Base_Config_Cjba.上周_建筑面积._ConfigCjbaMc()].doubls()) != 0 ? (temp_cjba_bz.Sum(m => m[Base_Config_Cjba.本周_成交金额._ConfigCjbaMc()].longs()) / temp_cjba_bz.Sum(m => m[Base_Config_Cjba.本周_建筑面积._ConfigCjbaMc()].doubls())).je_y() : 0; }; break;
                            case Base_Config_Cjba.上周_套内均价: { dr1[dt.Columns[j].ColumnName] = temp_cjba_sz.Sum(m => m[Base_Config_Cjba.上周_套内面积._ConfigCjbaMc()].doubls()) != 0 ? (temp_cjba_bz.Sum(m => m[Base_Config_Cjba.本周_成交金额._ConfigCjbaMc()].longs()) / temp_cjba_bz.Sum(m => m[Base_Config_Cjba.本周_套内面积._ConfigCjbaMc()].doubls())).je_y() : 0; }; break;
                            default: { dr1[dt.Columns[j].ColumnName] = ""; }; break;
                        }
                    }
                }
