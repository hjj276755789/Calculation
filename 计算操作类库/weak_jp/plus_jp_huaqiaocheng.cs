            ...
            try
            {
                ...
                foreach (var item in param)
                {
                    ...
                    if (item.ytcs[0] == "商务")
                    {
                        if (item.hxcs != null && item.hxcs.Length > 0)
                        {
                            for (int i = 0; i < item.hxcs.Length; i++)
                            {
                                ...
                                if (temp_cjba_sz != null && temp_cjba_sz.Count() > 0)
                                {
                                    ...
                                    dr["sztnjj"] = temp_cjba_sz.Sum(n => n["tnmj"].ints())!=0?(temp_cjba_sz.Sum(m => m["cjje"].longs()) / temp_cjba_sz.Sum(n => n["tnmj"].ints())).je_y():0;
                                }
                                else
                                {
                                    dr["szcjts"] = 0;
                                ...
                                if (temp_ba_bz != null)
                                {
                                    ...
                                    dr["bztnjj"] = temp_cjba_bz.Sum(n => n["tnmj"].ints())!=0?(temp_cjba_bz.Sum(m => m["cjje"].longs()) / temp_cjba_bz.Sum(n => n["tnmj"].ints())).je_y():0;
                            ...
                            foreach (var item_jp in item.jpxmlb)
                            {
                                if (item_jp.hxcs != null && item_jp.hxcs.Length > 0)
                                {
                                    for (int i = 0; i < item_jp.hxcs.Length; i++)
                                    {
                                        ...
                                        //上周本案认购数据
                                        var temp_ba_sz1 = temp_rgsj_bz1.FirstOrDefault();
                                        //本周本案认购数据
                                        ...
                                        if (temp_cjba_sz1 != null && temp_cjba_sz1.Count() > 0)
                                        {
                                            ...
                                            dr1["sztnjj"] = temp_cjba_sz1.Sum(n => n["tnmj"].ints())!=0?(temp_cjba_sz1.Sum(m => m["cjje"].longs()) / temp_cjba_sz1.Sum(n => n["tnmj"].ints())).je_y():0;
                                        }
                                        else
                                        {
                                            dr1["szcjts"] = 0;
                                        ...
                                        if (temp_ba_bz1 != null)
                                        {
                                            ...
                                            dr1["bztnjj"] = temp_cjba_bz1.Sum(n => n["tnmj"].ints())!=0?(temp_cjba_bz1.Sum(m => m["cjje"].longs()) / temp_cjba_bz1.Sum(n => n["tnmj"].ints())).je_y():0;
                        ...
                        Office_Tables.SetJP_HuaQiaoCheng_Table(page, dt, 2, null, null);
                        t.AddClone(page);
                    }
                    #endregion
                    #region 其他
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (temp_cjba_sz != null && temp_cjba_sz.Count() > 0)
                            {
                                ...
                                dr["szcjts"] = temp_cjba_sz.Sum(m => m["ts"].ints());
                                dr["szcjmj"] = temp_cjba_sz.Sum(m => m["jzmj"].ints());
                                dr["szcjje"] = temp_cjba_sz.Sum(m => m["cjje"].longs()).je_wy();
                                dr["sztnjj"] = temp_cjba_sz.Sum(n => n["tnmj"].ints())!=0?(temp_cjba_sz.Sum(m => m["cjje"].longs()) / temp_cjba_sz.Sum(n => n["tnmj"].ints())).je_y():0;
                            }
                            else
                            {
                            ...
                            if (temp_cjba_bz != null)
                            {
                                dr["bzcjts"] = temp_cjba_bz.Sum(m => m["ts"].ints());
                                dr["bzcjmj"] = temp_cjba_bz.Sum(m => m["jzmj"].ints());
                                dr["bzcjje"] = temp_cjba_bz.Sum(m => m["cjje"].longs()).je_wy();
                                dr["bztnjj"] = temp_cjba_bz.Sum(n => n["tnmj"].ints())!=0?(temp_cjba_bz.Sum(m => m["cjje"].longs()) / temp_cjba_bz.Sum(n => n["tnmj"].ints())).je_y():0;
                        ...
                        if (item.jpxmlb != null && item.jpxmlb.Count > 0)
                        {
                            foreach (var item_jp in item.jpxmlb)
                            {
                                if (item_jp.xfytcs != null && item_jp.xfytcs.Length > 0)
                                {
                                    ...
                                    {
                                        DataRow dr = dt.NewRow();
                                        dr["lpmc"] = item_jp.lpcs[0];
                                        dr["yt"] = item_jp.xfytcs[i];
                                        #region 数据准备
                                        //本周当前业态认购数据
                                        var temp_rgsj_bz = Cache_data_rgsj.bz.AsEnumerable().Where(m => m["xm"].ToString() == item_jp.lpcs[0] && m["yt"].ToString() == item_jp.xfytcs[i]);
                                        //本周当前业态备案数据
                                ...
                                else
                                {
                                    if (item_jp.ytcs != null && item_jp.ytcs.Length > 0)
                                    {
                                        DataRow dr = dt.NewRow();
                                        dr["lpmc"] = item_jp.lpcs[0];
                                        dr["yt"] = item_jp.ytcs[0];
                                        #region 数据准备
                                        string jpyt = item_jp.ytcs == null ? item.ytcs[0] : item_jp.ytcs[0];
                                        //本周当前业态认购数据
                                        var temp_rgsj_bz = Cache_data_rgsj.bz.AsEnumerable().Where(m => m["xm"].ToString() == item_jp.lpcs[0] && m["yt"].ToString() == jpyt);
                                        //本周当前业态备案数据
                                        var temp_cjba_bz = Cache_data_cjjl.bz.AsEnumerable().Where(m => m["lpmc"].ToString() == item_jp.lpcs[0] && m["yt"].ToString() == jpyt);
                                        //上周当前业态备案数据
                                        var temp_cjba_sz = Cache_data_cjjl.sz.AsEnumerable().Where(m => m["lpmc"].ToString() == item_jp.lpcs[0] && m["yt"].ToString() == jpyt);
                                        //本周本案认购数据
                                        var temp_ba_bz = temp_rgsj_bz.FirstOrDefault();
                                        #endregion
