            ...
            try
            {
                ...
                foreach (var item in param)
                {
                    ...
                    if (item.ytcs[0] == "商铺"||item.ytcs[0]=="商务")
                    {
                        ...
                        dt.Columns.Add(Base_Config_Cjba.上上上周_备案套数);//上上上周_备案套数
                        dt.Columns.Add(Base_Config_Cjba.上上上周_套内均价);//上上上周_套内均价

                        dt.Columns.Add(Base_Config_Cjba.上上周_备案套数);//上上周_备案套数
                        dt.Columns.Add(Base_Config_Cjba.上上周_套内均价);//上上周_套内均价
                    ...
                    else
                    {
                        ...
                        dt.Columns.Add(Base_Config_Rgsj.上上上周_认购套数);//上上上周_备案套数
                        dt.Columns.Add(Base_Config_Rgsj.上上上周_认购套内均价);//上上上周_套内均价
                        dt.Columns.Add(Base_Config_Rgsj.上上周_认购套数);//上上上周_备案套数
                        dt.Columns.Add(Base_Config_Rgsj.上上周_认购套内均价);//上上上周_套内均价
        ...
        public System.Data.DataTable GET_JPXM_BX(System.Data.DataTable dt, List<JP_JPXM_INFO> jpxm)
        {
            foreach (var item in jpxm)
            {
                if (item.ytcs[0] == "商铺")
                {
                        ...
                        var temp_cjba_ssz = Cache_data_cjjl.ssz.AsEnumerable().Where(m => m["lpmc"].ToString() == item.lpcs[0] && m["yt"].ToString() == item.ytcs[0]);
                        var temp_cjba_sssz = Cache_data_cjjl.sssz.AsEnumerable().Where(m => m["lpmc"].ToString() == item.lpcs[0] && m["yt"].ToString() == item.ytcs[0]);
                        //本周本案认购数据
                ...
                else if (item.ytcs[0] == "商务")
                {
                    if (item.hxcs.IsNotNull())
                    {
                        for (int i = 0; i < item.hxcs.Length; i++)
                        {
                            ...
                            var temp_ba_bz = Cache_data_cjjl.bz.AsEnumerable().Where(m => m["lpmc"].ToString() == item.lpcs[0] && m["hx"].ToString() == item.hxcs[i]);
                            var temp_ba_sz = Cache_data_cjjl.sz.AsEnumerable().Where(m => m["lpmc"].ToString() == item.lpcs[0] && m["hx"].ToString() == item.hxcs[i]);           
                            var temp_ba_ssz = Cache_data_cjjl.ssz.AsEnumerable().Where(m => m["lpmc"].ToString() == item.lpcs[0] && m["hx"].ToString() == item.hxcs[i]);
                            var temp_ba_sssz = Cache_data_cjjl.sssz.AsEnumerable().Where(m => m["lpmc"].ToString() == item.lpcs[0] && m["hx"].ToString() == item.hxcs[i]);
                            #endregion
                            dt.Rows.Add(GET_ROW(item.ytcs[0], dr1, dt, temp_rg_bz, temp_ba_bz, temp_ba_sz, temp_ba_ssz, temp_ba_sssz, item));
                        }
                    }
                    ...
                    else
                    {
                        ...
                        #endregion
                        dt.Rows.Add(GET_ROW(item.ytcs.Join(), dr1, dt, temp_rg_bz, temp_ba_bz, temp_ba_sz, temp_ba_ssz, temp_ba_sssz, item));
                ...
                else if (Base_Config_Cjba._备案数据.Contains(dt.Columns[j].ColumnName))
                {
                    ...
                    if (temp_cjba_sz != null)
                    {
                        switch (dt.Columns[j].ColumnName)
                        {
                            ...
                            case Base_Config_Cjba.上周_套内均价: { dr1[dt.Columns[j].ColumnName] = temp_cjba_sz.Sum(m => m[Base_Config_Cjba.上周_套内面积._ConfigCjbaMc()].doubls()) != 0 ? (temp_cjba_sz.Sum(m => m[Base_Config_Cjba.上周_成交金额._ConfigCjbaMc()].longs()) / temp_cjba_sz.Sum(m => m[Base_Config_Cjba.上周_套内面积._ConfigCjbaMc()].doubls())).je_y() : 0; }; break;
                        }
                    }
                    if (temp_cjba_ssz != null)
                    {
                        switch (dt.Columns[j].ColumnName)
                        {
                            case Base_Config_Cjba.上上周_备案套数: { dr1[dt.Columns[j].ColumnName] = temp_cjba_ssz.Sum(m => m[Base_Config_Cjba.上周_备案套数._ConfigCjbaMc()].ints()); }; break;
                            case Base_Config_Cjba.上上周_成交金额: { dr1[dt.Columns[j].ColumnName] = temp_cjba_ssz.Sum(m => m[Base_Config_Cjba.上周_成交金额._ConfigCjbaMc()].longs()); }; break;
                            case Base_Config_Cjba.上上周_建筑面积: { dr1[dt.Columns[j].ColumnName] = temp_cjba_ssz.Sum(m => m[Base_Config_Cjba.上周_建筑面积._ConfigCjbaMc()].doubls()); }; break;
                            case Base_Config_Cjba.上上周_套内面积: { dr1[dt.Columns[j].ColumnName] = temp_cjba_ssz.Sum(m => m[Base_Config_Cjba.上周_套内面积._ConfigCjbaMc()].ints()); }; break;
                            case Base_Config_Cjba.上上周_建面均价:
                                {
                                    var cjje = temp_cjba_ssz.Sum(m => m[Base_Config_Cjba.上上周_成交金额._ConfigCjbaMc()].longs());
                                    var jzmj = temp_cjba_ssz.Sum(m => m[Base_Config_Cjba.上上周_建筑面积._ConfigCjbaMc()].doubls());
                                    dr1[dt.Columns[j].ColumnName] = jzmj != 0 ? (cjje / jzmj).je_y() : 0;
                                }; break;
                            case Base_Config_Cjba.上上周_套内均价: { dr1[dt.Columns[j].ColumnName] = temp_cjba_ssz.Sum(m => m[Base_Config_Cjba.上上周_套内面积._ConfigCjbaMc()].doubls()) != 0 ? (temp_cjba_ssz.Sum(m => m[Base_Config_Cjba.上上周_成交金额._ConfigCjbaMc()].longs()) / temp_cjba_ssz.Sum(m => m[Base_Config_Cjba.上上周_套内面积._ConfigCjbaMc()].doubls())).je_y() : 0; }; break;
                        }
                    }
                    if (temp_cjba_sssz != null)
                    {
                        switch (dt.Columns[j].ColumnName)
                        {
                            case Base_Config_Cjba.上上上周_备案套数: { dr1[dt.Columns[j].ColumnName] = temp_cjba_sssz.Sum(m => m[Base_Config_Cjba.上上上周_备案套数._ConfigCjbaMc()].ints()); }; break;
                            case Base_Config_Cjba.上上上周_成交金额: { dr1[dt.Columns[j].ColumnName] = temp_cjba_sssz.Sum(m => m[Base_Config_Cjba.上上上周_成交金额._ConfigCjbaMc()].longs()); }; break;
                            case Base_Config_Cjba.上上上周_建筑面积: { dr1[dt.Columns[j].ColumnName] = temp_cjba_sssz.Sum(m => m[Base_Config_Cjba.上上上周_建筑面积._ConfigCjbaMc()].doubls()); }; break;
                            case Base_Config_Cjba.上上上周_套内面积: { dr1[dt.Columns[j].ColumnName] = temp_cjba_sssz.Sum(m => m[Base_Config_Cjba.上上上周_套内面积._ConfigCjbaMc()].ints()); }; break;
                            case Base_Config_Cjba.上上上周_建面均价:
                                {
                                    var cjje = temp_cjba_sssz.Sum(m => m[Base_Config_Cjba.上上上周_成交金额._ConfigCjbaMc()].longs());
                                    var jzmj = temp_cjba_sssz.Sum(m => m[Base_Config_Cjba.上上上周_建筑面积._ConfigCjbaMc()].doubls());
                                    dr1[dt.Columns[j].ColumnName] = jzmj != 0 ? (cjje / jzmj).je_y() : 0;
                                }; break;
                            case Base_Config_Cjba.上上上周_套内均价: { dr1[dt.Columns[j].ColumnName] = temp_cjba_sssz.Sum(m => m[Base_Config_Cjba.上上上周_套内面积._ConfigCjbaMc()].doubls()) != 0 ? (temp_cjba_sssz.Sum(m => m[Base_Config_Cjba.上上上周_成交金额._ConfigCjbaMc()].longs()) / temp_cjba_sssz.Sum(m => m[Base_Config_Cjba.上上上周_套内面积._ConfigCjbaMc()].doubls())).je_y() : 0; }; break;
                        }
                    }
                }
