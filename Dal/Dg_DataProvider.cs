...
select  t3.rwid,t2.nf,t2.zc,t2.mbid,t2.mbmc,t3.rwmc,case when t3.xzdz2 is not null then 1 else 0 end kfxz,t3.xzdz2 from(
...
where  t2.nf=@nf and t2.zc=@zc ";
            if (tj.IsNull())
            {
                sql += ") b2 where b1.mbbh =b2.mbid order by kfxz desc,b1.kfsbh";
            ...
            {
                sql += " ) b2 where  b1.mbbh =b2.mbid and b1.kfsmc like @kfsmc order by kfxz desc,b1.kfsbh";
