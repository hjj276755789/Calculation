...
namespace Calculation.Dal
{
    public class MySqlDbhelper
    {
        ...
        public static DataSet GetDataSet(string sql, params MySqlParameter[] commandParameters)
        {
            ...
            {
                //调用 PrepareCommand 方法，对 MySqlCommand 对象设置参数
                PrepareCommand(cmd, conn, null, CommandType.Text, sql, commandParameters);
                //调用 MySqlCommand 的 ExecuteReader 方法
                MySqlDataAdapter adapter = new MySqlDataAdapter();
                adapter.SelectCommand = cmd;
                DataSet ds = new DataSet();
        ...
        public static object ExecuteScalar(string sql, params MySqlParameter[] commandParameters)
        {
            MySqlCommand cmd = new MySqlCommand();
